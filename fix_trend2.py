...
async def patient_trend(patient_id: str, session: str | None = Cookie(default=None)):
    require_doctor(session)
    # Try trend DB first (analysed_at column)
    rows = []
    try:
        with _get_trend_db() as conn:
            rows = conn.execute(
        ...
        rows = []
    # Fall back to main clinical.db (created_at column)
    if not rows:
...
if OLD in text:
    ...
    print("✅  patient-trend endpoint updated with clinical.db fallback")
else:
    print("❌  Pattern not found — trying line replacement...")
    ...
    print("✅  Column names patched directly")
